...
$.when(
        $.getJSON('/mgtn3simulator/js/enemySetiting.json?query=' + Date.now(), function(jsonData) {
            enemy = jsonData;
        }),
        $.getJSON('/mgtn3simulator/js/allySetiting.json?query=' + Date.now(), function(jsonData) {
            ally = jsonData;
        })
    ).done(
        (function initialize() {
...
function allyActionButtonMdown(event) {
    ...
    var _effect = ally.party[0].skills[skill_index].effect;
    ...
    switch (_effect.type) {
        ...
        case ALLY_SKILL_TYPE.HEAL:
            if (ally.party[0].max_hp < ally.party[0].cur_hp + _effect.amount) {
                // 加算した結果が最大HPより大きい場合は最大HPを現在HPに設定する
                ally.party[0].cur_hp = ally.party[0].max_hp;
            } else {
                // 加算した結果が最大HP以下の場合は加算した結果を現在HPに設定する
                ally.party[0].cur_hp = ally.party[0].cur_hp + _effect.amount;
            }
            // HTML上に反映する
            replace('#ally_hp', ally.party[0].cur_hp + ' / ' + ally.party[0].max_hp);
            break;
